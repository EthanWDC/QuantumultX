...
[rewrite_local]
# 获取SAP Parking cookie
https:\/\/www\.sapdigitallunch\.com\/spq\/mobile\/parking\/init url script-request-header https://autotask.333234.xyz/sapparking/submit_cookie.js?code=XXXX

[mitm] 
hostname= www.sapdigitallunch.com
*/

const $ = API('sapparking_getcookie');
const date = new Date();

if ($.env.isRequest) {
  const code = $request.params['code'];
  $.notify("SAP Parking监控", "", code);
  const cookie = $request.headers['cookie']||$request.headers['Cookie'];
  $.notify("SAP Parking监控", "", cookie);
  if (code && cookie) {
    //$.notify("SAP Parking监控", "", `获取cookie成功 🎉`);
  } else {
  	//$.notify("SAP Parking监控", "", `获取cookie失败`);
  }
  
  const reqData = {
    url: 'https://autotask.333234.xyz/sapparking/update_cookie.php',
    body:JSON.stringify({"code":code,"cookie":cookie})
  };
  ...
  $.http.post(reqData)
    ...
    .catch((err) => ($.msgBody = `cookie更新失败 ‼️‼️\n${err || err.message}`))
    .finally(async () => {
      $.notify('SAP Parking监控', ``, $.msgBody);
      $.done();
    })
} 

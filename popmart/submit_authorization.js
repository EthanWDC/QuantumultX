...
[rewrite_local]
# 获取Authorization
https:\/\/spirit\-gateway\.paquapp\.com\/v1\/daemon\/users\/get\-state url script-request-header https://raw.githubusercontent.com/EthanWDC/QuantumultX/master/popmart/submit_authorization.js

[mitm] 
hostname= spirit-gateway.paquapp.com
*/

const $ = API('popmart_getauth');
const date = new Date();

if ($.env.isRequest) {
  const union_id = $request.headers['union_id'];
  const authorization = $request.headers['authorization'];
  if (union_id && authorization) {
    //$.notify("泡泡玛特助手", "", `获取密钥成功 🎉`);
  } else {
  	//$.notify("泡泡玛特助手", "", `获取密钥失败`);
  }
  
  const reqData = {
    url: 'https://autotask.333234.xyz/popmart/update_authorization.php',
    body:JSON.stringify({'union_id':union_id,'authorization':authorization})
  };
  ...
  $.http.post(reqData)
    ...
    .catch((err) => ($.msgBody = `密钥更新失败 ‼️‼️\n${err || err.message}`))
    .finally(async () => {
      $.notify('泡泡玛特助手', ``, $.msgBody);
      $.done();
    })
} 
